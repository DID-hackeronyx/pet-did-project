...
const Ipfs = () => {
  ...
  const onChangeImageFile: ChangeEventHandler<HTMLInputElement> = async(e) => {
    ...
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BACK_URL}/api/pet?unique_key=${e.target.files[0].name}`);
    ...
    if (imageFile) {
      ...
      try {
        ...
        const response2 = await axios.post(
          ...
          {
            name, m_records, r_date , did:address , petId :'response.~'
          }
